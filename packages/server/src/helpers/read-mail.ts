...
import { IMapSettings } from "../services/mail-service.js";
import { simpleParser, ParsedMail } from "mailparser";
import {
  Box as MailBox,
  ImapMessage,
  ImapMessageAttributes,
  Config as ImapConfig,
} from "imap";
...
export class ReadMail extends EventEmitter implements IReadMail {
  ...
  constructor(
    private imapCfg: IMapSettings,
    mailaccount: string,
    mailaccountPwd: string,
    latestMailUid: number = 0
  ) {
    super();
  ...
  public async FetchMails(): Promise<void> {
    ...
    const searchText: string =
      this.uidLatest === 0 ? "1:*" /* all mails */ : `${this.uidLatest + 1}:*`;
    const mailBoxes = await this.getAllMailBoxes();
    ...
    if (
      !uuids ||
      uuids.length === 0 ||
      (uuids.length === 1 && uuids[0] === this.uidLatest)
    ) {
      // No new mails found!
      this.connection.end();
    } else {
      ...
      try {
        let fetchRequest = this.connection.fetch(uuids, {
          bodies: "" /*'BODY.PEEK[]', struct: true*/,
        });
        // fetchRequest.once('error', (error) => { console.log(); });
        const fetch = eventToPromise.multi(fetchRequest, ["end"], ["error"]);
        fetchRequest.on("message", (msg: ImapMessage, uid: number) =>
          this.parseMail(msg, uid, mailQueue)
        );
  ...
  private parseMail(
    msg: ImapMessage,
    uid: number,
    queque: Map<number, ParsedMail>
  ) {
    // Get the MAIL message (body and headers)
    msg.once(
      "body",
      async (stream: Stream, info: imapConnection.ImapMessageBodyInfo) => {
        // simpleParser is used to convert stream into mail message (with attachments)
        // See https://nodemailer.com/extras/mailparser/
        simpleParser(stream, (_err: any, parsedMail: ParsedMail) => {
  ...
  private searchUUID(searchText: string): Promise<number[]> {
    return new Promise((resolve, reject) => {
      this.connection.search(
        [["UID", searchText]],
        (err: Error, uidNumbers: number[] /* UID numbers */) => {
          if (err) reject(err.message);
          else resolve(uidNumbers);
        }
      );
    });
  }
  ...
  private openMailBox(): Promise<MailBox> {
    return new Promise((resolve, reject) => {
      this.connection.openBox(
        MailBoxName,
        false /* readonly */,
        (err: Error, box: MailBox) => {
          if (err) reject(err.message);
          else return resolve(box);
        }
      );
    });
  }
  ...
  private getAllMailBoxes(): Promise<imapConnection.MailBoxes> {
    return new Promise((resolve, reject) => {
      this.connection.getBoxes(
        (error: Error, mailboxes: imapConnection.MailBoxes) => {
          if (error) reject();
          else resolve(mailboxes);
        }
      );
    });
  }
