...
$SwaggerSever = "http://localhost:9090/api/resources/swagger.yaml"
...
$swagger_definition = resolve-path ".\SwaggerDefintion.yaml"
...
$ErrorCode = Invoke-WebRequest $SwaggerSever -OutFile "SwaggerDefintion.yaml"
if ($?){
   Write-Host "Generate TypeScript FETCH api client"
   # /opt/swagger-codegen
   docker run -v ${PWD}:/mounted swaggerapi/swagger-codegen-cli generate  -i /mounted/SwaggerDefintion.yaml -l typescript-fetch -o /mounted/generated_code -c /mounted/swagger-config.json
